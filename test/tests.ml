...
let and_prey_eaten (prey : prey_record) (g : int) (a : int) (c : int) (t : int)
    : bool =
  prey.goldfish = g && prey.anemone = a && prey.clownfish = c && prey.turtle = t
...
let shark7 =
  ...
  (prey7.clownfish = 1 && prey7.anemone = 0 && prey7.goldfish = 0
 && prey7.turtle = 0)
...
let shark8 =
  ...
  and_prey_eaten prey8 0 0 0 0 && get_health game8 Shark = 40.

let _ = buy_fish_game game8 Goldfish 2

let shark8' =
  let prey8 = get_eaten_prey game8 in
  and_prey_eaten prey8 2 0 0 0 && get_health game8 Shark = 30.
...
let shark9 =
  ...
  (prey9.goldfish = 1 || prey9.anemone = 1 || prey9.clownfish = 1
 || prey9.turtle = 1)
...
let shark10 =
  ...
  and_prey_eaten prey10 2 0 0 0 && get_health game10 Shark = 100.

let shark10' =
  let prey10' = get_eaten_prey game10 in
  and_prey_eaten prey10' 0 0 0 0 && get_health game10 Shark = 90.
...
let money_tests =
  [
    ...
    ( "Price of 8 clownfish" >:: fun _ ->
      assert_equal 80. (price_fish Clownfish 8) );
    ...
    ( "Playermoney start of game1" >:: fun _ ->
      assert_equal 100. (get_playermoney game1) );
    ...
    ("Broke medicine game3" >:: fun _ -> assert_equal true (med_broke game3));
...
let population_tests =
  [
    (* extinct tests *)
    ( "Extinct shark game1" >:: fun _ ->
      assert_equal true (species_extinct game1 Shark) );
    ( "Not extinct shark game2" >:: fun _ ->
      assert_equal false (species_extinct game2 Shark) );
    ( "Extinct goldfish game2" >:: fun _ ->
      assert_equal true (species_extinct game3 Goldfish) );
...
let game_tests =
  [
    ...
    (* End score *)
    ("End score game2" >:: fun _ -> assert_equal true (end_score game2 > 0));
...
let user_input_tests =
  [
    (* parse tests *)
    ("Parse empty string" >:: fun _ -> assert_equal [] (parse ""));
    ("Parse ABCDEF" >:: fun _ -> assert_equal [ "ABCDEF" ] (parse "ABCDEF"));
    ("Parse abcdef" >:: fun _ -> assert_equal [ "ABCDEF" ] (parse "abcdef"));
    ("Parse whitespace" >:: fun _ -> assert_equal [] (parse "   "));
    ...
    (* parse_input tests *)
    ("Parse input empty string" >:: fun _ -> assert_equal Dunno (parse_input ""));
    ("Parse input whitespace" >:: fun _ -> assert_equal Dunno (parse_input "  "));
    ...
    ("Parse input Fe ed" >:: fun _ -> assert_equal Dunno (parse_input "Fe ed"));
    ( "Parse input invalid Manual 5" >:: fun _ ->
      assert_equal Dunno (parse_input "Manual 5") );
    ("Parse input feed" >:: fun _ -> assert_equal Feed (parse_input "Feed"));
    ...
    ( "Parse input feed alternate uppercase lowercase" >:: fun _ ->
      assert_equal Feed (parse_input "fEeD") );
    ("Parse input Buy" >:: fun _ -> assert_equal Buy (parse_input "Buy"));
    ...
    ("Parse input Pass" >:: fun _ -> assert_equal Pass (parse_input "Pass"));
    (* parse_species tests *)

...
type tank = { tank_type : string; fish_list : fish list; capacity : int }
...
let start_game (i : int) : player_state =
  { round = 1; money = 100; tank = new_tank "Nursery"; max_rounds = i }
...
let fish_bio (f : fish) : string =
  "  " ^ f.name ^ "       " ^ f.species ^ "       " ^ string_of_int f.age
  ^ "       " ^ string_of_int f.hunger

(**Prints a fish_list using fish_bio. Helper function for print_tank*)
let rec print_fish_list (lst : fish list) : string =
  match lst with [] -> "" | h :: t -> fish_bio h ^ "\n" ^ print_fish_list t
...
let make_fish (n : string) (sp : string) (color : string) : fish =
  { name = n; species = sp; color; age = 0; hunger = 20 }
